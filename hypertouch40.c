...
struct al3050_bl_data {
	...
	struct gpio_desc *gpiod;
        struct gpio_desc *gpioclk;
        struct gpio_desc *gpiomosi;
        struct gpio_desc *gpiocs;
	int last_brightness;
	int power;
        int rfa_en;
};
...
gpiod_direction_output(pchip->gpioclk,0);
ndelay(T_START_NS);
gpiod_direction_output(pchip->gpioclk,1);
ndelay(T_START_NS);
count--;
...
{       struct device *dev = &pdev->dev;
	...
	int ret, x;
        u32 value;
        uint16_t data_lcd[] = {0x0FF,0x1FF,0x198,0x106,0x104,0x101,
0x008,0x110,0x021,0x10D,0x030,0x102,0x031,0x100,
0x040,0x110,0x041,0x155,0x042,0x102,0x043,0x184,
0x044,0x184,0x050,0x178,0x051,0x178,0x052,0x100,
0x053,0x177,0x057,0x160,0x060,0x107,0x061,0x100,
0x062,0x108,0x063,0x100,0x0A0,0x100,0x0A1,0x107,0x0A2,0x10C,0x0A3,0x10B,
...
0x053,0x110,0x054,0x113,0x0FF,0x1FF,0x198,0x106,0x104,0x107,
0x018,0x11D,0x017,0x122,0x002,0x177,0x026,0x1B2,
0x0E1,0x179,0x0FF,0x1FF,0x198,0x106,0x104,0x100,
0x03A,0x160,0x035,0x100,0x011,0x100,0xffff,0x029,0x100,};
	...
	if (IS_ERR(bl))
        ...
        pchip->gpioclk = gpiod_get(dev, "clk", GPIOD_ASIS);
        pchip->gpiomosi = gpiod_get(dev, "mosi", GPIOD_ASIS);
        pchip->gpiocs = gpiod_get(dev, "cs", GPIOD_ASIS);

        if (!IS_ERR(pchip->gpioclk) && !IS_ERR(pchip->gpiomosi) && !IS_ERR(pchip->gpiocs))
          ...
          for(x=0; x < sizeof(data_lcd)/sizeof(uint16_t); x++ )
	{
		...
		if (data_lcd[x] == 0xffff)
			{mdelay(100);}
			else { al3050_write_lcd_value(pchip,data_lcd[x]);}
...
/*                              clk-gpios = <&gpio 27 GPIO_ACTIVE_HIGH>;
                                ...
                                cs-gpios = <&gpio 18 GPIO_ACTIVE_HIGH>;
 */

        gpiod_direction_input(pchip->gpiocs);
        gpio_unexport(pchip->gpiocs);
	gpio_free(pchip->gpiocs);

        gpiod_direction_input(pchip->gpioclk);
        gpio_unexport(pchip->gpioclk);
        gpio_free(pchip->gpioclk);


        gpiod_direction_input(pchip->gpiomosi);
        gpio_unexport(pchip->gpiomosi);
        gpio_free(pchip->gpiomosi);
